...
import { PriceCalculator } from '@/app/todayMenu/cart/priceCalculator';
...
export class CartService {
  ...
  static async updateCartItemQuantity(cartItemId: string, quantity: number): Promise<boolean> {
    try {
      ...
      // 옵션 정보 가져오기
      const optionIds = (cartItem.cart_item_options as Array<{ menu_option_id: string }>)
        ?.map(opt => opt.menu_option_id) || [];
    ...
    try {
      ...
      const formatted = data?.map((item: unknown) => {
        ...
        return {
          ...
          menu_option_ids: cartItem.cart_item_options?.map(
